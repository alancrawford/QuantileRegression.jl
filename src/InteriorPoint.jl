...
macro atleastversion(x)
    if eval(x) >= v"0.7-"
        return :(LinearAlgebra.qr(Q*X, Val(true)))
    elseif eval(x) >= v"0.4-"
...
z = r .* (r .> 0.0)
w = z - r
gap = BLAS.dot(c, x) - BLAS.dot(y, b) + BLAS.dot(w, u)
...
for it = 1:max_it
    ...
    AQtF = LinearAlgebra.qr(Q*X, Val(true)) # PE 2004
    rhs = Q*r        # "
    dy = AQtF\rhs   # "
    dx = q.*(X*dy - r)
    ds = -dx
    ...
    if min.(fp, fd) < 1.0
        ...
        # Update mu
        mu = BLAS.dot(z, x) + BLAS.dot(w, s)
        g = BLAS.dot(z + fd*dz, x + fp*dx) + BLAS.dot(w + fd*dw, s + fp*ds)
        mu = mu * (g / mu)^3 / (2 * n)
